...
- 数据库以嵌入二进制的 `000001_baseline` 建立破坏性发布基线，并通过兼容的 `000002` 至 `000013_oauth_publisher_trust` 加法迁移演进；当前 `0.5.0-rc.1` 要求 schema version 13，可从正式 `0.3.0` 的 schema version 3 依次迁移。服务启动只校验 schema，不再隐式迁移。

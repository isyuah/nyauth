...
- 应用所有者与管理员可查看 7/30/90 天 OAuth 成功率、流程趋势、活动授权数和受限失败诊断；诊断不保存 Token、Secret、授权码、用户身份字段或回调查询参数
- 管理员应用列表新增搜索、客户端类型、Grant、发布者状态、归属和活动排序，并展示所有者、活动授权及最近七天成功/失败摘要
- 用户中心 OAuth 授权改为服务端搜索、状态筛选和分页，主列表使用紧凑摘要，Scope、Claim、身份快照及历史放入详情抽屉
- Provider 管理新增无凭据端点预检和交互诊断历史；交互诊断不创建账号、绑定身份或登录会话，也不保存上游 Token 或声明值

### 变更

- 正式发布 `0.7.0`；兼容迁移 `000018_oauth_operations` 将 schema version 从 17 提升到 18，可从正式 `0.6.0` 的 schema version 17 直接升级

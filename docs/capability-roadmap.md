...
> 状态：更新于 2026-08-03。本文档记录"网站自身能力"的增强方向与取舍结论，按优先级排序。
...
| OAuth 运营与诊断 | Phase E 已实现：应用级统计与失败诊断、可扩展应用/授权列表、Provider 预检与交互诊断 | 已实现，待验收 |
...
```
当前 0.7.0-dev: Phase E 已实现，schema 为 18，等待集中回归和用户验收
下一步: 验收应用运营、授权列表和 Provider 诊断后形成 0.7.0 检查点

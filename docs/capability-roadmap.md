...
| 运行时设置（部分配置免重启） | Phase C2–C5 已实现运营策略、受控媒体迁移、认证生命周期与 OAuth 客户端策略；部署拓扑配置仍需重启 | 已完成当前阶段 |
...
```
当前 0.4.0-dev: Phase C1 已验收，Phase C2–C5 已实现；C5 等待用户验收
下一步: Phase C6 事务邮件模板、预览/测试与站点公告
后续: Phase C7 动态日志级别、告警阈值与 OTLP 运营参数；自动更新、/api/v1、用户组和事件 Webhook 继续推迟
```

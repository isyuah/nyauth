...
> 状态：更新于 2026-08-04。本文档记录"网站自身能力"的增强方向与取舍结论，按优先级排序。
...
真正值得免重启的是**运营型设置**：品牌（标题/主色/Logo/favicon）、注册策略、邮件发送配置、限流阈值、审计保留时长等——这些本质是"数据"而不是"部署形态"。当前实现：

- 通用 `runtime_settings` 已承载品牌、注册、安全、访问保护、生命周期和 OAuth 客户端策略。六组设置统一使用 revision CAS，管理变更走 CSRF、近期重新认证、固定代码级限流和事务审计。品牌设置生成受控色阶，允许管理员在自动、白色和黑色之间选择主色文字，并支持浅/深 Logo 和 favicon；浅色、深色与跟随系统由访问者浏览器本地保存，不写入账号或站点默认。认证界面和新入队事务邮件读取同一运行时品牌快照。
...
## 7. OAuth 运营与诊断（Phase E 已完成）

- 应用所有者和管理员可查看 7/30/90 天协议操作成功/失败、活动授权数、按流程与阶段拆分的趋势，以及最近 90 天的受限失败明细。
- 失败原因使用代码内有界目录；回调地址删除 query/fragment，Scope 规范化，且不保存 Token、Secret、授权码、PKCE verifier、用户字段或原始内部错误。日聚合保留 400 天。
- 管理员应用列表支持搜索、Public/Confidential、Grant、发布者状态、归属和排序；用户应用仍受权威配额控制。所有者数据页通过后端校验 Owner，越权访问统一返回 404。
- 用户授权列表改为服务端搜索、有效/信息变化/需重新授权/未使用状态筛选和分页；主列表只显示可扫描摘要，Scope、Claim、应用链接及授权快照在详情抽屉中展示。
- Provider 支持无凭据端点预检和真实交互诊断。交互诊断绑定当前管理员会话，不创建用户、不绑定身份、不建立新会话，也不保存上游 Token、原始 Claim 或字段值；结果保留 90 天。

## 8. 公告与站内消息（Phase F）

- 保留“全站横幅”作为短时、全局、可关闭的即时运营提示；持久公告使用独立数据模型，支持草稿、发布/计划展示、归档、受众、置顶和安全 Markdown。
- 公告不为每个用户预先复制数据；`announcement_reads` 按用户和公告 revision 保存惰性读回执。已发布内容发生修改后会重新进入未读状态。
- 个人站内消息使用代码内有界类型目录，当前接入密码、TOTP、Passkey、外部身份、OAuth 授权撤销和应用发布者可信状态变化。通知失败不回滚已经成功的安全操作，但会写服务端诊断日志。
- SSE 只传输按数据库重新计算的未读计数，不承载消息正文；断线后自动退回轮询，PostgreSQL 始终是事实来源。
- 公告和通知正文只接受服务端安全渲染的 Markdown，拒绝原始 HTML、图片和非 HTTPS/非站内链接。
- `/dashboard/messages` 将公告和个人消息按统一时间线查询，支持分类、已读状态、级别、文本与时间范围筛选。合并排序与分页在 PostgreSQL 中完成，不使用两个前端分页结果进行不完整的本地混排。

## 9. 事件 Webhook（P2）——插件系统的替代品
...
```
当前 0.8.0-dev: Phase F 已完成，schema 为 19；自动化、真实 PostgreSQL/Redis、schema 18→19 和本地容器升级验证通过
下一步: 从通知偏好、公告阅读统计、OAuth Client Secret 无停机轮换或账户生命周期中选择独立阶段

...
| 运行时设置（部分配置免重启） | Phase C2 已实现动态限流、会话生命周期、审计保留和客户端配额；部署拓扑配置仍需重启 | 已完成当前阶段 |
...
- 通用 `runtime_settings` 已承载品牌、注册、安全、访问保护和生命周期策略。五组设置统一使用 revision CAS，管理变更走 CSRF、近期重新认证、固定代码级限流和事务审计。
...
- 浏览器会话绝对期限、近期认证期限和审计保留天数可动态调整；审计缩短只保存策略，删除仍由受控 `maintenance` 执行。
...
```
当前 0.4.0-dev: Phase C1 已验收，Phase C2 运行时运营策略已实现并进入验收
下一步: 单独设计 C3 存储后端受控迁移，不把连接与密钥配置直接热切换

...
`0.3.0-rc.1` 建立了 schema version 1 的破坏性 release baseline，不能从更早的开发数据库升级。正式 `0.3.0` 通过兼容迁移演进到 schema version 3；`0.4.0-rc.1` 通过兼容的 `000004` 至 `000009_runtime_observability` 演进到 schema version 9，`0.5.0-dev` 通过 `000010_human_verification` 演进到 schema version 10。升级前保存旧 digest，备份 PostgreSQL、头像媒体和 master key，并阅读目标版本的迁移说明：

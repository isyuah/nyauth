...
func TestRuntimeMediaCandidateEncryptionAndMigrationStateMachine(t *testing.T) {
	...
	if _, err := schema.pool.Exec(ctx, `INSERT INTO media_storage_instances(instance_id,version,started_at,heartbeat_at,loaded_revision) VALUES($1,'0.5.0',NOW(),NOW(),$2)`, otherInstanceID, state.Revision); err != nil {

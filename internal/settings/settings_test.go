...
func TestSecurityFallsBackToEnrollmentEnabledAndOptionalAdminMFA(t *testing.T) {
	...
	if !security.TOTPEnabled || security.RequireMFAForAdmins {
	...
	if got := manager.Security(); got.TOTPEnabled || got.RequireMFAForAdmins {
...
func TestLoadWithoutDatabaseKeepsDefaults(t *testing.T) {
	...
	if security := manager.Security(); !security.TOTPEnabled || security.RequireMFAForAdmins {

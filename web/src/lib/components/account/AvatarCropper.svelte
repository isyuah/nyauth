<script lang="ts">
  ...
  let zoomID = $derived(`${componentId}-zoom`);
  ...
  async function submit() {
    ...
    try {
      ...
      error = cause instanceof Error ? cause.message : `${subject}上传失败，请稍后重试。`;
    } finally {
      uploading = false;
    }
  }
  ...
  async function removeCurrent() {
    ...
    try {
      ...
      error = cause instanceof Error ? cause.message : `${subject}删除失败，请稍后重试。`;
    } finally {
      removing = false;
    }
  }
...
<div class="space-y-3">
  <div class="flex flex-wrap items-center gap-3">
    <label class="inline-flex cursor-pointer items-center gap-2 rounded-nya-sm border border-nya-border bg-nya-surface px-3 py-2 text-small font-semibold text-nya-text-primary hover:bg-nya-surface-muted aria-disabled:cursor-not-allowed aria-disabled:opacity-60" aria-disabled={disabled}>
      <ImagePlus size={16} /> 选择并裁剪{subject}
    ...
    {#if currentUrl && onremove}
      <Button variant="ghost" size="sm" disabled={disabled} loading={removing} onclick={removeCurrent}><Trash2 size={15} /> 删除{subject}</Button>
    {/if}
  </div>
...
<Modal bind:open title={`裁剪${subject}`} description="拖动图片选择区域，或使用滑块调整缩放" size="lg">
  <div class="grid gap-5 md:grid-cols-[minmax(0,1fr)_160px]">
    <div>
      <button type="button" aria-label={`${subject}裁剪区域，可拖动图片，或使用方向键移动图片，按住 Shift 可加速`} onkeydown={moveWithKeyboard} class="block w-full max-w-[420px] rounded-nya-md p-0 outline-none focus:ring-2 focus:ring-nya-primary">
        <canvas
          bind:this={canvas}
      ...
      <div class="mt-4">
        <label for={zoomID} class="mb-1.5 block text-small font-semibold text-nya-text-primary">缩放 {zoom.toFixed(2)}×</label>
        <input id={zoomID} class="w-full accent-nya-primary" type="range" min="1" max="3" step="0.01" bind:value={zoom} />
      </div>
    ...
    <div>
      <p class="mb-2 text-small font-semibold text-nya-text-primary">{previewShape === 'circle' ? '圆形' : '圆角方形'}预览</p>
      <div class="h-32 w-32 overflow-hidden border border-nya-border bg-nya-surface-muted {previewShape === 'circle' ? 'rounded-full' : 'rounded-nya-md'}">
        <canvas bind:this={circleCanvas} class="h-full w-full" aria-label={`${subject}预览`}></canvas>
      </div>
  ...
  <div class="mt-5 flex justify-end gap-2">
    ...
    <Button variant="primary" loading={uploading} onclick={submit}><Upload size={15} /> 上传{subject}</Button>
  </div>
</Modal>

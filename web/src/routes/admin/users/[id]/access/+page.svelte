  ...
  {#snippet children()}
    <div class="grid gap-4 xl:grid-cols-2">
      <section class="rounded-nya-card border border-nya-border bg-nya-surface p-5 shadow-nya-card">
        ...
        {#if identities.length === 0}<p class="rounded-nya-sm bg-nya-surface-muted p-3 text-body text-nya-text-tertiary">未绑定外部身份</p>{:else}<div class="space-y-2">{#each identities as identity}<div class="flex items-center gap-3 rounded-nya-sm bg-nya-surface-muted p-3"><Badge variant="info">{identity.provider}</Badge><div class="min-w-0 flex-1"><p class="truncate text-body text-nya-text-primary">{identity.external_username || identity.external_id}</p><p class="truncate text-small text-nya-text-tertiary">{identity.external_email || '未提供邮箱'}</p></div><Button variant="ghost" size="sm" ariaLabel={`解绑 ${identity.provider} 身份`} onclick={() => requestIdentityRemoval(identity)}><Trash2 size={14} /> 解绑</Button></div>{/each}</div>{/if}
      </section>
